	...
	level: {
		...
		generate: function () {
			...
			for(s = 0; s < waySpaceCoords.length; s++) {
				...
				ways.push(new way(y1 * 32, y2 * 32)); // y, l, h, w
			...
			for(var w in ways) {
				...
				for(i = y1; i < y2; i++) {
					...
					var speed 		= this.value/Math.floor(genRnd(0.1, 0.3));
...
var frock = function (x, y, w, h, d) {
	...
	this.move = function (direction) {
		...
		if(!wouldHit) {
			...
			// audioObj.play('move');
			var tmp = Math.floor(genRnd(1, 100));
			if(tmp == 50) audioObj.play('car');
		}
...
var way = function (y1, y2, l, w) {
	
	this.y1		= y1;
	this.y2		= y2 || this.y + 64;
	...
	this.draw = function (destContext /* destination context */) {
		...
		if(this.l) {
			...
			// defaults for asphalt lines

			this.line = { // default values for lines on way
				height	: this.h/20, 
				width	: this.w/32,
				left	: this.w/16 // default margin left
			};

			this.ctx.fillStyle = "rgba(255,255,255,0.3)";

			for(i = 0; (i * this.line.width) + (this.line.left * i) + this.line.left < this.w; i++) {
				this.ctx.fillRect(
					(i * this.line.width) + (this.line.left * i) + this.line.left,
				 	this.h/2 - this.line.height/2,
					this.line.width, 
					this.line.height
				);
			}
		...
		// put pre rendered canvas on canvas 
		destContext.drawImage(this.tpl, 0, this.y1);
...
window.onkeydown = function (event) {
	...
	if(!game.pause) {
		if(frockObj.y - frockObj.h * 3 < window.scrollY) scrollAnimateTo(frockObj.y - frockObj.h*3, 100);
